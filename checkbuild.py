...
def check_build(args):
    ...
    logging.debug("Repo Dir: %s" % repo_dir)
    ...
    (ret, stdout, stderr) = run_cmd("./bootstrap-configure",
                                    ...
                                    cwd=repo_dir)
    if ret:
        process_failure(stdout, stderr)
        sys.exit("'bootstrap-configure' terminated with non-zeor(%d)" % ret)

    # make
    (ret, stdout, stderr) = run_cmd("make", cwd=repo_dir)
    if ret:
        process_failure(stdout, stderr)
        sys.exit("'make' terminated with non-zeor(%d)" % ret)

    process_success(stdout, stderr)
    logging.info("'checkbuild' terminated with zero(0)")
...
def main():
    ...
    if 'GITHUB_TOKEN' not in os.environ:
        sys.exit("ERROR: Cannot find GITHUB_TOKEN from environment variables")
